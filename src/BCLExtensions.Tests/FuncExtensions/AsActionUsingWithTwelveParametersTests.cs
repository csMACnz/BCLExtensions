...
namespace BCLExtensions.Tests.FuncExtensions
{
    public class AsActionUsingWithTwelveParametersTests
    {
        ...
        {
            Assert.DoesNotThrow(() => SampleFunction(42, "Test", true, 9.876m, byte.MaxValue, 12, "Foo", false, 3.33333m, ByteValue, 9000, "Test"));
        }

        [Fact]
        public void ResultNotNull()
        {
            Func<int, string, bool, decimal, byte, int, string, bool, decimal, byte, int?, string, decimal> function = SampleFunction;

            var action = function.AsActionUsing(12, "12", false, 5.4321m, byte.MaxValue, 42, "Bar", true, 66666.3m, ByteValue, 1600, "Class");
        ...
        public void InternalFunctionExecutes()
        {
            ...
            Func<int, string, bool, decimal, byte, int, string, bool, decimal, byte, int?, string, decimal> function = (p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,p11,p12) =>
            {
                internalFunctionWasCalled = true;
                return 42m;
            };
            var action = function.AsActionUsing(12,"24",true, 3.14m, byte.MaxValue, 16, "Fizz", false, 452.6m, ByteValue, 65536, "Malloc");
            action();
        ...
        public void InternalFunctionCapturesCorrectParameters()
        {
            ...
            var action = function.AsActionUsing(expectedParameter1, expectedParameter2, expectedParameter3, expectedParameter4, expectedParameter5, expectedParameter6, expectedParameter7, expectedParameter8, expectedParameter9, expectedParameter10, expectedParameter11, expectedParameter12);
            action();
        ...
        private decimal SampleFunction(int parameter1, string parameter2, bool parameter3, decimal parameter4, byte parameter5, int parameter6, string parameter7, bool parameter8, decimal parameter9, byte parameter10, int? parameter11, string parameter12)
        {
            return 42m;
        }
    }
