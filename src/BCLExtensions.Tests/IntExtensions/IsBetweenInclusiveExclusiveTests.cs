...
namespace BCLExtensions.Tests.IntExtensions
{
    public class IsBetweenInclusiveExclusiveTests
    {
        ...
        {
            Func<int, InclusiveInteger, ExclusiveInteger, bool> isBetween = BCLExtensions.IntExtensions.IsBetween;

            Assert.Throws<InvalidOperationException>(isBetween.AsActionUsing(0, 100.Inclusive(), 20.Exclusive()).AsThrowsDelegate());
        }
    }
}
